...
function Clear-XEFolder
{
	...
	if (Test-Path -Path $folderPath)
	{
		try
		{
			# Remove all items within the folder, excluding those starting with 'S'
			Get-ChildItem -Path $folderPath -Recurse -Force |
			Where-Object { $_.Name -notlike 'S*' } | Remove-Item -Force -Recurse
			Write-Log "Folder 'XE${StoreNumber}901' (Urgent Messages) cleared successfully, excluding 'S*' files." "green"
		}
		catch
		{
			Write-Log "An error occurred during initial clearing of 'XE${StoreNumber}901': $_" "red"
		}
	}
	else
	{
		...
		$job = Start-Job -Name "ClearXEFolderJob" -ScriptBlock {
			...
			while ($true)
			{
				try
				{
					if (Test-Path -Path $folderPath)
					{
						# Get all items except those matching exclusions
						Get-ChildItem -Path $folderPath -Recurse -Force |
						Where-Object {
							$exclusionMatched = $false
							foreach ($exclusion in $exclusions)
							{
								if ($_.Name -like $exclusion)
								{
									$exclusionMatched = $true
									break
								}
							}
							-not $exclusionMatched
						} | Remove-Item -Force -Recurse
					}
				}
				catch
				{
					# Suppress errors to avoid cluttering logs; can be enhanced if needed
				}
				
				Start-Sleep -Seconds $checkIntervalSeconds
			}
		} -ArgumentList $folderPath, $checkIntervalSeconds, $exclusions
		
		# Write-Log "Background job 'ClearXEFolderJob' started to monitor and clear 'XE${StoreNumber}901' every $checkIntervalSeconds seconds, excluding items starting with specified patterns." "green"
	}
	catch
	{
...
@FMT(CMP,"@DBSELECT(SELECT F1042 FROM FCT_TAB WHERE F1063 = @WIZGET(F1063))=",'@FMT(CHR,27)');

@WIZSET(F1000=@DBSELECT(SELECT F1000 FROM FCT_TAB WHERE F1063 = @WIZGET(F1063)));
@WIZSET(F1042=@DBSELECT(SELECT F1042 FROM FCT_TAB WHERE F1063 = @WIZGET(F1063)));
@WIZSET(F1043=@DBSELECT(SELECT F1043 FROM FCT_TAB WHERE F1063 = @WIZGET(F1063)));
@WIZSET(F1044=@DBSELECT(SELECT F1044 FROM FCT_TAB WHERE F1063 = @WIZGET(F1063)));
@WIZSET(F1045=@DBSELECT(SELECT F1045 FROM FCT_TAB WHERE F1063 = @WIZGET(F1063)));
@WIZSET(F1046=@DBSELECT(SELECT F1046 FROM FCT_TAB WHERE F1063 = @WIZGET(F1063)));
@WIZSET(F1047=@DBSELECT(SELECT F1047 FROM FCT_TAB WHERE F1063 = @WIZGET(F1063)));
@WIZSET(F1050=@DBSELECT(SELECT F1050 FROM FCT_TAB WHERE F1063 = @WIZGET(F1063)));
@WIZSET(F1051=@DBSELECT(SELECT F1051 FROM FCT_TAB WHERE F1063 = @WIZGET(F1063)));
@WIZSET(F1052=@DBSELECT(SELECT F1052 FROM FCT_TAB WHERE F1063 = @WIZGET(F1063)));
@WIZSET(F1053=@DBSELECT(SELECT F1053 FROM FCT_TAB WHERE F1063 = @WIZGET(F1063)));
@WIZSET(F1054=@DBSELECT(SELECT F1054 FROM FCT_TAB WHERE F1063 = @WIZGET(F1063)));
@WIZSET(F1055=@DBSELECT(SELECT F1055 FROM FCT_TAB WHERE F1063 = @WIZGET(F1063)));
@WIZSET(F1064=@DBSELECT(SELECT F1064 FROM FCT_TAB WHERE F1063 = @WIZGET(F1063)));
@WIZSET(F1081=@DBSELECT(SELECT F1081 FROM FCT_TAB WHERE F1063 = @WIZGET(F1063)));

@WIZRPL(F1042=@FMT(CMP,"@WIZGET(F1042)=",NULL,@WIZGET(F1042)));
@WIZRPL(F1043=@FMT(CMP,"@WIZGET(F1043)=",NULL,@WIZGET(F1043)));
@WIZRPL(F1044=@FMT(CMP,"@WIZGET(F1044)=",NULL,@WIZGET(F1044)));
@WIZRPL(F1045=@FMT(CMP,"@WIZGET(F1045)=",NULL,@WIZGET(F1045)));
@WIZRPL(F1046=@FMT(CMP,"@WIZGET(F1046)=",NULL,@WIZGET(F1046)));
@WIZRPL(F1047=@FMT(CMP,"@WIZGET(F1047)=",NULL,@WIZGET(F1047)));
@WIZRPL(F1054=@FMT(CMP,"@WIZGET(F1054)=",NULL,@WIZGET(F1054)));
@WIZRPL(F1055=@FMT(CMP,"@WIZGET(F1055)=",NULL,@WIZGET(F1055)));

@CMD_DEPLOY
DELETE FROM FCT_TAB WHERE F1063 = @WIZGET(F1063);

@CMD_DEPLOY
INSERT INTO FCT_TAB (F1063,F1000,F1042,F1043,F1044,F1045,F1046,F1047,F1050,F1051,F1052,F1053,F1054,F1055,F1064,F1081) 
VALUES (@WIZGET(F1063),'@WIZGET(F1000)',@WIZGET(F1042),@WIZGET(F1043),@WIZGET(F1044),@WIZGET(F1045),@WIZGET(F1046),@WIZGET(F1047),'@WIZGET(F1050)','@WIZGET(F1051)','@WIZGET(F1052)','@WIZGET(F1053)',@WIZGET(F1054),@WIZGET(F1055),'@WIZGET(F1064)','@WIZGET(F1081)');
			...
			$storeButton4.Add_Click({
					...
					$storeButton4.Add_Click({
							...
							$runButton.Add_Click({
									...
									# Attach the DoWork event handler using the Add_DoWork method
									$bgWorker.Add_DoWork({
											param ($sender,
												$e)
									...
									# Attach the RunWorkerCompleted event handler using the Add_RunWorkerCompleted method
